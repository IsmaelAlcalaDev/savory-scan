...
export const useOptimizedRestaurants = ({
  searchQuery = '',
  userLat,
  userLng,
  maxDistance = 50,
  ...
  const fetchRestaurants = useCallback(async (signal: AbortSignal) => {
    try {
      ...
      console.error('useOptimizedRestaurants: Error:', err);
      setError(err instanceof Error ? err.message : 'Error al cargar restaurantes');
      setRestaurants([]);
      setLoading(false);
    }
  }, [fetchKey, searchQuery, userLat, userLng, maxDistance, cuisineTypeIds, priceRanges, isHighRated, selectedEstablishmentTypes, selectedDietTypes, isOpenNow, isBudgetFriendly]);
  ...
  useEffect(() => {
    ...
    fetchTimeoutRef.current = setTimeout(() => {
      if (!currentController.signal.aborted) {
        fetchRestaurants(currentController.signal);
      }
    }, 300);
