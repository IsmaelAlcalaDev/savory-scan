...
export default function DishFavoriteButton({
  ...
  const handleToggle = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    await toggleFavorite(dishId, savedFrom, handleLoginRequired);
  };

  const liked = isFavorite(dishId);
  ...
  if (showCount) {
    ...
    return (
      <button
        ...
        aria-pressed={liked}
        aria-label={liked ? "Quitar plato de favoritos" : "Añadir plato a favoritos"}
      >
        {loading ? (
