...
import { Leaf, Wheat, Milk, Heart, Flame, Clock, Plus, MapPin } from 'lucide-react';
...
interface AllDishCardProps {
  ...
  onDishClick?: (dish: DishWithRestaurant) => void;
  onLoginRequired: () => void;
}

export default function AllDishCard({ dish, onDishClick, onLoginRequired }: AllDishCardProps) {
  const navigate = useNavigate();

  const getDietIcon = (dish: DishWithRestaurant) => {
    const icons = [];
            ...
            {dish.distance_km && (
              <>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>{dish.distance_km.toFixed(1)} km</span>
                </div>
              </>
            )}
          </div>
